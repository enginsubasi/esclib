...
## 28. A check that can be computed as the bytes arrive — DONE 16/09/2026

Every CRC and checksum here took a pointer and a length, and that is the wrong shape for two of the jobs they are actually used for. A frame arrives on an interrupt a byte at a time, so either the check is computed as it comes or the payload is held twice. A firmware image is verified while it is being written to flash, where holding it is not an option at all. Both want the same thing: the value so far, plus one byte.

**One Update per implementation rather than per polynomial**, which is the decision in it worth arguing. `crc16` and `crc16Alt` are one polynomial computed two ways, and so are `crc32` and `crc32Alt`; each of the four got its own streaming form rather than the streaming path being table-driven only. The reason is the case the feature exists for: a firmware update on a small part is exactly where somebody copied `crc16Alt` out to save 512 bytes, and giving that caller a table-driven Update would hand the flash straight back.

**The tables moved to file scope.** They were `static const` inside their functions, so a streaming entry point would have grown a second copy — a kilobyte of it in `crc32`'s case. They are read only, so they land in `.rodata` and the storage rule in `check.sh` is untouched.

**Only two of the five checksums got one.** `xor`, `sum8` and `sum16` are one operation a byte that a caller writes inline with nothing to get wrong, and an entry point for them would be symmetry for its own sake. `Fletcher16` and `Adler32` carry a modulus and an accumulator that feeds on the other, which is what a hand-written version gets wrong. Neither needed a struct: each already packs its state into the value it returns, so the state is the result and the starting point is a number. `CHECKSUM_ADLER32_SEED` is **one**, the classic mistake, and the test pins it by the exact amount a zero seed is out by rather than by "differs".

Eight mutations, all caught. The test checks every prefix of a 256-byte buffer against the whole-buffer form rather than one length, because an Update that agrees at 256 bytes and nowhere else is still wrong, and the ways these go wrong — a seed applied twice, a byte taken into the wrong end — show at particular lengths rather than at all of them.

Writing that test also found a mistake of mine rather than the module's. The first version asserted that seeding Adler32 at zero leaves the result one too low. It does not: the first accumulator ends one low and the second ends one low for every byte, so over sixteen bytes it is out by one in the low half and sixteen in the high. The assertion says that now, and it is a stronger check than the one I meant to write.

## 29. The claim cordic made, checked — DONE 16/09/2026

`cordic` went in because two files had written down an absence, and when it was done CLAUDE.md said the absence was closed: a caller who has to design a `biquad` at boot on a part with no FPU now has a cosine to do it with. Nothing tested that sentence. A claim a library makes about itself and never exercises is the kind of thing that turns out to be false quietly, and this one had an obvious way to check it.

**The design stack.** `Integration_Test` now designs a 50 Hz notch for a 1 kHz loop with **no float between the sample rate and `biquadIniti32`**: the corner becomes a binary angle by integer arithmetic, `cordicSinCos` supplies the cosine, `q16Div` produces the five coefficients. Checked two ways, because either alone would be weak. The coefficients agree with `biquadInitNotch`'s to within eight parts in 65536 — the float designer runs in the test as a reference, exactly as a host would. And the filter is then *measured*: a tone at the notch is removed and one well outside it passes, both tones generated with `cordic`, since a part with no FPU has no other way to make one. A coefficient comparison alone would not have caught a filter installed with its numerator and denominator swapped; a behavioural check alone would not have caught a design that was a different filter from the one asked for.

**The phasor stack.** The other absence `cordic` was written for: `complexi32` has no polar pair and says why. Two Q16 phasors multiply into an apparent power, `cordicPolar` reads it back as a length and an angle, and the power factor is `cordicCos` of that angle. 230 volts at zero degrees and 5 amps at minus thirty give 1150 at minus thirty and a power factor of 0.866, which is the textbook answer to a textbook question, arrived at with no float on the part.

**And a sample**, `sample/NotchNoFpu`, which is the design written out for a reader to copy. It prints its coefficients beside the ones a host computes and then runs a 400-count measurement with 300 counts of hum through the filter: 699 counts in, 400 out. That number is the point of the whole exercise.

Writing it found two mistakes, both mine and both in the test rather than the library. The binary angle I used for minus thirty degrees was minus sixty — a third of `0x100000000` rather than a twelfth — which the power factor check caught and the angle check could not, because the angle check compared the recovered angle against the same wrong constant. And the reference coefficients I wrote into the sample's output were invented rather than computed; they are computed now. Neither was a defect in a module, but both are the reason a test that only compares a thing against itself is worth nothing.

## Everything on this list is built

`softtimer`, the `comstxetx` transparency and integrity work, `checksum`, `interp`, the `basicmath` scalars, `ramp` and `encoder`, and after them the widths and the Q16 variants, `biquad`'s and `mathLerp`'s included, and then `dcMotor`'s speed control, `crc8`, `pack`, `fir` and `goertzel`, the mutation harness that keeps the tests honest, `q16`, `sched`, `fsm`, and finally the four stubs, a measurement pass over what the fixed point variants cost, the composition tests and worked examples that had never existed, the last three width gaps, a sanitizer run that found six pieces of undefined behaviour hiding behind correct answers, cordic, which two other files had been asking for in writing, commodbus, which is the framing an industrial bus actually uses, cobs, which is the other answer to the question comstxetx answers, text, the stdio this library does not have, rms, statistic in the shape an interrupt can call, a streaming form for every check in the tree, and the composition tests that check what cordic claimed. Nothing is outstanding, and for the first time nothing is reserved either.
