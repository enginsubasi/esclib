/**
  ...
  * 16/09/2026 crc32Update and crc32AltUpdate added, for a caller @n
  *            checking an image as it arrives rather than after. The @n
  *            table moved to file scope, where both forms share it. @n
  *
  ******************************************************************************
  */
...
/*
 * The table for the 0x04C11DB7 polynomial, at file scope so that crc32
 * and crc32Update share one copy of it. A kilobyte, which is the largest
 * constant in this library and the reason crc32Alt exists.
 */
...
/**
 * @brief   Takes one byte into a CRC32 that is being computed a piece at a
 *          time.
 *
 * @param[in]   crc     the value so far, CRC32_SEED before the first byte.
 * @param[in]   data    the byte.
 *
 * @return  The value including that byte.
 *
 * @note    CRC32_SEED run through this once per byte gives exactly what crc32
 *          gives over the same bytes. This is the form for a firmware image
 *          checked as it is written, where the whole buffer form would need
 *          the image in memory.
 */
...
/**
 * @brief   Takes one byte into a CRC32, computed bit by bit.
 *
 * @param[in]   crc     the value so far, CRC32_SEED before the first byte.
 * @param[in]   data    the byte.
 *
 * @return  The value including that byte.
 *
 * @note    The same value crc32Update gives, in eight shifts rather than a
 *          lookup, for the caller who took crc32Alt to save the kilobyte.
 */
