/**
  ...
  * 15/09/2026 The i32 width is added. A ring of signed samples @n
  *            had to be cast in and out of the u32 one, and a @n
  *            wrong cast there is silent. @n
  *
  ******************************************************************************
  */
 ...
 * The signed width.
 *
 * A ring of signed samples is ordinary — a converter reading that swings about
 * zero, a control error, an encoder delta — and the u32 buffer forces a caller
 * holding one to cast on the way in and on the way out. Either cast is silent
 * when it is wrong, and the value comes back as four thousand million rather
 * than as minus one.
 *
 * The bodies below are the u32 ones unchanged, because nothing in them depends
 * on the element type: they index an array and compare indices. Only the
 * signatures differ, which is the whole reason this is a width of circBuf
