...
Stateful modules use their own name (`maf`, `emaf`, `median`, `biquad`, `slew`, `deadband`, `alphabeta`, `pid`, `ramp`, `circBuf`, `comat`, `comstxetx`, `bininp`, `hysteresis`, `complex`, `interp`, `hc595`, `hc597`, `dcMotor`). Check with `nm` after adding anything:
...
Twenty-four test programs cover every module that has functions, and **every one of the 214 exported symbols is referenced by at least one of them**. The only files with no test are `comsec`, `comsafe`, `comgenbuf` and `matrixlib`, which have nothing to test — see the known gaps above. Check that coverage claim still holds after adding an exported function:
