...
Stateful modules use their own name (`maf`, `emaf`, `median`, `biquad`, `slew`, `deadband`, `alphabeta`, `pid`, `ramp`, `circBuf`, `comat`, `comstxetx`, `bininp`, `hysteresis`, `complex`, `interp`, `encoder`, `hc595`, `hc597`, `dcMotor`). Stateless ones use the module name too: `crc8`/`crc8Dallas`, `checksum*`, `pack*`. Check with `nm` after adding anything:
